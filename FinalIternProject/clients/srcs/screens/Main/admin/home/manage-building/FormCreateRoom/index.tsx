...
import {AppStackParamList, FormsAddListRoom, FormsCreateRoom, TAppNavigation} from '@types';
import {
  ColorsStatic,
...
// };
type FormCreateRoomRouteProp = RouteProp<AppStackParamList, RouteMain.FormCreateRoom>;
export const FormCreateRoom: React.FC = () => {
  const {styles} = useStyles(stylesheet);
  
  const route = useRoute<FormCreateRoomRouteProp>();
  const {item, onRoomCreated} = route.params || {};
  ...
  const onSubmit = async (data: FormsCreateRoom) => {
    try {
  ...
  return (
    <Box flex={1}>
      ...
      <FormProvider {...forms}>
        ...
        <FormButtonFooter
          onCallbackSave={(item: FormsCreateRoom) => handlePressAdd(item)}
