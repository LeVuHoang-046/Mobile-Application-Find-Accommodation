...
import { FormsAddListRoom, FormsUpdateRoom } from '@types';
import { stylesheet } from '../style';

type FormRoomInformationProps = {
  room?: FormsAddListRoom | null;
}

export const FormRoomInformation: React.FC<FormRoomInformationProps> = memo(({room}) => {
  ...
  const {
    ...
    formState: {errors},
    clearErrors,
  } = useFormContext<FormsUpdateRoom>();
  ...
  //       setValue(key as keyof FormsUpdateRoom, room[key as keyof FormsUpdateRoom]);
  //     });
  //     setImageRoom(room.imageRoom || []);
  //     setVideoRoom(room.videoRoom || []);
  //   }
  // }, [room, setValue]);
...
imageRoom.forEach(image => {
  // const formattedPath = `file://${image.path.replace(/\\/g, '/')}`;
