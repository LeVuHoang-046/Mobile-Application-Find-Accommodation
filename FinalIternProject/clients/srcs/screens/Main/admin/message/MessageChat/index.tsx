...
const MessageChatScreen: React.FC<PerformanceNavigationHOC> = ({ navigateFinish }) => {
  ...
  const fetchMessages = async () => {
    try {
      const response = await axios.get(`http://192.168.31.22:3000/api/chat/${senderId}/${receiverId}`);
  ...
  const handleSendMessage = () => {
    ...
    axios.post('http://192.168.31.22:3000/api/chat', messageToSend)
      .then(() => {
      ...
      <Box flex={1}>
        ...
        {navigateFinish ? (
          <Box flex={1}>
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} style={{ flex: 0.8, padding: scaler(8), backgroundColor:ColorsStatic.white }}>
            {messages.map((msg: any, index: any) => (
              ...
              >
                <Box 
                  ...
                  maxW="80%"
                >
                  <TextApp size={FontSize.Font13} weight={700} color={msg.sender_id === senderId ? '#fff' : '#000'}>
                    {msg.message}
                  </TextApp>
                </Box>
              </Box>
...
export const MessageChat = performanceNavigation(MessageChatScreen);
